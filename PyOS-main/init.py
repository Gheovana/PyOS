...
class PCB:
    ...
    def __init__(self, nome, prioridade=1):
        ...
        self.nome = nome
        self.estado = "PRONTO"  # Estados possíveis: PRONTO, EXECUTANDO, TERMINADO, BLOQUEADO
    ...
    while True:
        try:
            ...
            if acao == "exit":
                ...
                print("  block [PID]  - Bloqueia um processo (simula espera por E/S)")
                ...
                print(f"{'PID':<6} | {'NOME':<12} | {'PRIO':<6} | {'ESTADO':<12} | {'CICLOS'}") 
                print("-" * 55)
                for p in tabela_processos:
                    print(f"{p.pid:<6} | {p.nome[:12]:<12} | {p.prioridade:<6} | {p.estado:<12} | {p.ciclos_restantes}")
                ...
                if len(comando) > 1:
                    try:
                        ...
                        for p in tabela_processos:
                            if p.pid == alvo:
                                p.estado = "BLOQUEADO"
                                print(f"[Kernel] PID {alvo} bloqueado.")
                                found = True
                                break
                        if not found:
                ...
                if len(comando) > 1:
                    try:
                        ...
                        found = False
                        for p in tabela_processos:
                            if p.pid == alvo: # Corrigido: de p.id para p.pid
                                p.estado = "PRONTO"
                                print(f"[Kernel] PID {alvo} desbloqueado.")
                                found = True
                                break
                        if not found:
                            print(f"[Kernel] PID {alvo} não encontrado.")
                ...
                while tabela_processos:
                    escalonador_tick()
                    time.sleep(0.5)  # Pequena pausa para simular o tempo real
